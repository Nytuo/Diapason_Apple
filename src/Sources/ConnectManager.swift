...
class ConnectManager: ObservableObject {
    ...
    // ── Discovery (client: find desktop/other devices) ──────────────────────────
    ...
    // ── Registered devices (they connected to us = we can control them back) ────
    @Published var registeredDevices: [ConnectDevice] = []
    @Published var registeredStatuses: [String: ConnectStatus] = [:]

    // ── Server (host: receive commands + registrations) ─────────────────────────
    ...
    // MARK: Registration (announce ourselves so the remote can control us back)
