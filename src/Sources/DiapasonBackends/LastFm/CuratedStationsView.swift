// Diapason — curated genre/chart stations in Discover (Last.fm-backed).
...
/// Horizontal row of curated stations (Top 100 + genres). Shown only when a
/// Last.fm API key is configured.
struct CuratedStationsSection: View {
    private let stations = CuratedStation.all

    var body: some View {
        Group {
            if LastFmChartClient.isConfigured {
                    ...
                    ScrollView(.horizontal, showsIndicators: false) {
                        HStack(spacing: DiapasonSpacing.m) {
                            ForEach(stations) { station in
                                NavigationLink {
                                    CuratedStationDetailView(station: station)
                                } label: {
                                    CuratedStationCard(station: station)
                                }
                                .buttonStyle(.plain)
                            }
                        }
...
private struct CuratedStationCard: View {
    let station: CuratedStation
    var body: some View {
        VStack(alignment: .leading, spacing: DiapasonSpacing.xs) {
            RoundedRectangle(cornerRadius: DiapasonCornerRadius.large, style: .continuous)
                .fill(LinearGradient(colors: [Color.accent.opacity(0.8), Color.purple.opacity(0.6)],
                                     startPoint: .topLeading, endPoint: .bottomTrailing))
                .frame(width: 150, height: 150)
                .overlay(Image(systemName: station.kind == .top100 ? "chart.bar.fill" : "guitars.fill")
...
/// Resolves a curated station's tracks against the library (else YouTube) and
/// plays them — mirrors `LBPlaylistDetailView`.
struct CuratedStationDetailView: View {
    let station: CuratedStation
    ...
    var body: some View {
        Group {
            if isLoading {
                ...
                EmptyStateView(systemImage: "music.note.list", title: "Nothing to play")
            } else {
                List {
                    Section {
                        Button { play(startIndex: 0) } label: {
    ...
    private func resolve() async {
        ...
        let tracks = await station.fetchTracks()
